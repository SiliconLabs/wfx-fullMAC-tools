 ...
 *
 * \return IP address if possible or NULL if not.
 */
...
/**
 * \brief Find a requested IP address in a DHCP packet.
 *
 * \param pbuf_in the pbuf containing the a DHCP packet.
 * \param ipaddr a pointer to place the value ip addr.
 */
...
/**
 * \brief Receive DHCP discover and request, and respond to 
 * them 
 */
static void dhcpserver_fn(void *arg, struct udp_pcb *dhcp_pcb_recv, struct pbuf *pbuf_in, const ip_addr_t *client_addr, uint16_t port)
{
