...
#define WSM_API_RESERVED1_SIZE                          3         
#define WSM_API_LOCAL_DEV_NAME_SIZE                     32        
#define WSM_API_SECONDARY_DEVICE_TYPE_LIST_SIZE         16        
...
typedef enum wsm_mib_id_e {
        ...
        WSM_MIB_ID_P2P_DEVICE_INFO                 = 0x101d,      /*4.35 P2PDeviceInfo*/
        WSM_MIB_ID_SET_WCDMA_BAND                  = 0x101e,      /*4.36 SetWCDMABand*/
        WSM_MIB_ID_GRP_SEQ_COUNTER                 = 0x101f,      /*4.37 GroupTxSequenceCounter*/
        ...
        WSM_MIB_ID_TSF_COUNTER                     = 0x1023,      /*4.41 TSF Counter Value*/
...
typedef struct __attribute__((__packed__)) WsmHiResetFlags_s {
        ...
        uint8_t  ResetAllInt : 1;                  /*Set high to reset all interfaces, not only the one indicated by link_id*/
        uint8_t  Reserved : 6;       
        uint8_t  Reserved2[WSM_API_RESERVED2_SIZE];   /*Reserved2 0*/   
} WsmHiResetFlags_t;

typedef struct __attribute__((__packed__)) WsmHiScanType_s {
        uint8_t  Type : 1;                         /*bit0 : 0=foreground/1=background type: Wsm*/
        uint8_t  Mode : 2;                         /*bit1-2 : 0=single scan, 1=auto periodic scan, 2=auto quality scan     : scan is done only if reception quality (tbd) is below a threshold*/
        uint8_t  Reserved : 5;                     /*Reserved*/      
} WsmHiScanType_t;

typedef struct __attribute__((__packed__)) WsmHiScanFlags_s {
        uint8_t  Fbg : 1;                          /*1 - forced background scan.     Forced background scan means if the station cannot enter the power-save mode,    it will be force to perform a background scan.     Only valid when ScanType is background scan.*/
        uint8_t  Split : 1;                        /*A split scan method is to be used*/
        uint8_t  Pre : 1;                          /*Preamble type: 0 : Long, 1 : Short*/
        uint8_t  TxMod : 1;                        /*11n Tx mode: 0 : Mixed, 1 : Greenfield*/
        uint8_t  UseMacAddr1 : 1;                  /*Use the second mac address*/
        uint8_t  Reserved : 3;                     /*Reserved*/      
} WsmHiScanFlags_t;

typedef struct __attribute__((__packed__)) WsmHiAutoScanParam_s {
        uint16_t Interval;                         /*Interval period in TUs that the device will re-execute the requested scan.    Maximum value supported by the device is 256 s.    Applicable to auto-scan ScanType only.*/
        uint8_t  Reserved;                         /*Reserved*/      
        int8_t   RssiThr;                          /*Scan RSSI Threshold in dBm. Scan reports with lower RSSI value will be discarded and not passed to the host. A value of zero (default) will make all reports pass.*/
} WsmHiAutoScanParam_t;

typedef struct __attribute__((__packed__)) WsmHiSsidDef_s {
        uint32_t SSIDLength;                       /*Length of the SSID*/
        uint8_t  SSID[WSM_API_SSID_SIZE];          /*SSID character 0*/
} WsmHiSsidDef_t;

typedef struct __attribute__((__packed__)) WsmHiQueueId_s {
        uint8_t  QueueId : 2;                      /*Transmit queue ID (as defined in set Tx queue params) specifies the Access Category Index (ACI) of the queue. 0 - BE ; 1 - BG ; 2 - VI ; 3 - VO type: Wsm*/
        uint8_t  PerStaId : 4;                     /*Bits 5 to 2 : peer STA id (from 1 to 14) type: WsmPmMode*/
        uint8_t  Reserved : 2;                     /*Bits 7 to 6: reserved*/
} WsmHiQueueId_t;

typedef struct __attribute__((__packed__)) WsmHiTxFlags_s {
        uint8_t  Startexp : 1;                     /*Start expiry time from : 0 - the first Tx attempt (default) 1 - the receipt of the Tx request*/
        uint8_t  Pta : 3;                          /*PTA priority*/  
        uint8_t  Txrate : 3;                       /*Tx rate retry policy*/
        uint8_t  Offset : 1;                       /*Packet Frame Control field offset : 0 - at Frame offset 1 - at Frame offset plus 2 bytes*/
} WsmHiTxFlags_t;

typedef struct __attribute__((__packed__)) WsmHiHtTxParameters_s {
        uint8_t  FrameFormat : 4;                  /*Transmission frame format. 0: Non-HT. 1: Mixed format. 2: Greenfield format. Other values reserved*/
        ...
        uint8_t  Unused : 1;                       /*set to 0*/      
        uint8_t  Stbc : 1;                         /*STBC requirement for this frame. 0: STBC not allowed. 1: STBC allowed*/
        uint8_t  Reserved1;                        /*Reserved*/      
        uint8_t  Aggregation : 1;                  /*aggregation 11N*/
        uint8_t  AggrFirst : 1;                    /*aggregation first MPDU*/
        uint8_t  AggrLast : 1;                     /*aggregation last MPDU*/
        uint8_t  Reserved2 : 5;                    /*Reserved*/      
        uint8_t  Reserved3;                        /*Reserved*/      
} WsmHiHtTxParameters_t;

typedef struct __attribute__((__packed__)) WsmHiTxResultFlags_s {
        uint8_t  Aggr : 1;                         /*Only valid for the WSM_SUCCESS status. 0 - Frame was not sent aggregated. 1 - Frame was sent aggregated*/
        uint8_t  Requeue : 1;                      /*Valid only when status is WSM_REQUEUE. 1 - Host should re-queue this frame later. 0 - Host should not re-queue this frame. type: Wsm*/
        uint8_t  AckPolicy : 2;                    /*Only valid for the WSM_SUCCESS status. 00b - Normal ACK ; 01b - No ACK ; 10b - No explicit acknowledgement ; 11b - Block ACK*/
        uint8_t  TxopLimit : 1;                    /*The TXOP limit for the ACI was temporarily increased to allow this frame to transmit.*/
        uint8_t  MoreTxForMulti : 1;               /*seen in FW .h*/ 
        uint8_t  TxResumePresent : 1;              /*seen in FW .h*/ 
        uint8_t  TxState : 1;                      /*Indicates whether the TX state structure is available in the confirmation structure */
        uint8_t  Reserved;                         /*Reserved*/      
} WsmHiTxResultFlags_t;

typedef struct __attribute__((__packed__)) WsmHiRxFlags_s {
        uint8_t  Encryp : 3;                       /*Frame encryption type type: Wsm*/
        uint8_t  InAggr : 1;                       /*Bit 3 : 1 - Frame was part of an aggregation*/
        uint8_t  FirstAggr : 1;                    /*Bit 4 : 1 - Frame was first in an aggregation (Only valid if bit 03 = 1)*/
        uint8_t  LastAggr : 1;                     /*Bit 5 : 1 - Frame was last in an aggregation (Only valid if bit 03 = 1)*/
        uint8_t  Defrag : 1;                       /*Bit 6 : 1 - Indicates a defragmented frame*/
        uint8_t  Beacon : 1;                       /*Bit 7 : 1 - Indicates a beacon frame*/
        uint8_t  Tim : 1;                          /*Bit 8 : 1 - Indicates the STA bit beacon TIM field*/
        uint8_t  Bitmap : 1;                       /*Bit 9 : 1 - Indicates that the virtual bitmap of the beacon frame contains multicast bit*/
        ...
        uint8_t  Meas : 1;                         /*Bit 13 : 1 - Indicates the frame received during a measurement process*/
        uint8_t  Ht : 1;                           /*Bit 14 : 1 - Indicates the frame received as an HT packet*/
        uint8_t  Stbc : 1;                         /*Bit 15 : 1 - Indicates the frame received with STBC*/
        uint8_t  MatchStationid : 1;               /*Bit 16 : 1 - Indicates the address 1 field matches dot11StationId*/
        uint8_t  GroupAddr : 1;                    /*Bit 17 : 1 - Indicates the group address present in the address 1 field*/
        uint8_t  BcAddr : 1;                       /*Bit 18 : 1 - Indicates the broadcast address present in the address 1 field*/
        ...
        uint8_t  KeyIndex : 4;                     /*Bits 23 to 20 : KeyIndex used for decryption*/
        uint8_t  Timestamp : 1;                    /*1 - An 8-byte of timestamp is appended at the end of a beacon frame or a probe-response frame.             The timestamp is the device TSF time recorded at the time when the start of the Timestamp field in the beacon frame or the probe-response frame is received.             This information can be used to find out the TSF difference between our BSS and the foreign BSS.             If this bit is set, the message size includes the length of the timestamp.*/
        uint8_t  PeerStaId : 4;                    /*peer STA id (from 1 to 14)*/
        uint8_t  Reserved : 2;                     /*Reserved*/      
        uint8_t  BeaconFiltering : 1;              /*used internally to apply beacon filtering*/
} WsmHiRxFlags_t;

typedef struct __attribute__((__packed__)) WsmHiJoinFlags_s {
        uint8_t  Sync : 1;                         /*Bit 0 = 0 - synchronized join ; 1 - unsynchronized join type: WsmMode*/
        uint8_t  P2P : 1;                          /*Bit 1 = 0 - The BSS owner is a legacy WLAN AP ; 1 - The BSS owner is a P2P GO type: WsmMeasureType*/
        uint8_t  ForceNoBeacon : 1;                /*Bit 2 = 1 - Force to join BSS with the BSSID and the SSID specified without waiting for beacons. The ProbeForJoin parameter is ignored. type: WsmMeasureType*/
        uint8_t  Priority : 1;                     /*Bit 3 = 1 - Give probe request/response higher priority over the BT traffic type: Wsm*/
        uint8_t  UseMacAddrIf : 1;                 /*Bit 4 - Index of MAC address to use*/
        uint8_t  ForceWithInd : 1;                 /*Bit 5 - Force with Join Complete Indication  type: WsmPreamble*/
        uint8_t  ChangeP2PInt : 1;                 /*Bit 6 - Use a different P2P Interface Address*/
        uint8_t  Reserved2 : 1;                    /*Reserved for future*/
} WsmHiJoinFlags_t;
...
typedef struct __attribute__((__packed__)) WsmHiBssFlags_s {
        uint8_t  LostCountOnly : 1;                /*Bit 0 = 1: Only update the beacon lost count limit and reset the internal beacon lost counter. type: Wsm*/
        uint8_t  Reserved : 7;                     /*Reserved type: WsmPmMode*/
} WsmHiBssFlags_t;

typedef struct __attribute__((__packed__)) WsmHiChannelMode_s {
        uint8_t  Reserved2 : 1;                    /*Reserved type: Wsm*/
        uint8_t  Transmit : 1;                     /*1 - The STA will not transmit any further frames until the channel switch has completed.*/
        uint8_t  Enhanced : 1;                     /*1 - Enhanced switch-channel mode. Extra eight fields are available in the request structure (those that come after NewChannelNumber). type: WsmAckplcy*/
        uint8_t  Reserved1 : 5;                    /*Reserved*/      
} WsmHiChannelMode_t;

typedef struct __attribute__((__packed__)) WsmHiSwitchFlags_s {
        uint8_t  Abort : 1;                        /*1 - Abort the switch channel operation. The other fields are ignored.*/
        uint8_t  Indic : 1;                        /*1 - No start indication. In the enhanced mode, the device normally sends two channel-switch indications             - one indicates the new channel is switched and another indicates the original channel is restored.             If the host only uses channel switch to listen, it can set this flag and tell the device not to send the first indication.*/
        uint8_t  PlcpPreamb : 2;                   /*Specifies the PLCP preamble type used*/
        uint8_t  Reserved1 : 1;                    /*Reserved*/      
        uint8_t  SyncBss : 1;                      /*1 - Synchronize the start of channel switch with a foreign BSS. If Bit 3 is set, the fields are relevant. Bit 3 has priority over Bit 1.*/
        uint8_t  SyncEnd : 1;                      /*1 - Synchronize channel switch at end. The device aborts the operation in the new channel if an activity in the original channel is scheduled.*/
        uint8_t  SyncStart : 1;                    /*1 - Synchronize channel switch on start. The device starts to switch to the new channel when it is idle.*/
        uint8_t  Reserved2;                        /*Reserved*/      
} WsmHiSwitchFlags_t;

typedef struct __attribute__((__packed__)) WsmHiMode_s {
        uint8_t  StartMode : 2;                    /*Bit 1-0 : Start mode 0: MiniAP 1: P2P GO 2: generic channel  type: WsmHiState*/
        uint8_t  Unused : 2;         
        uint8_t  IndexMacUse : 1;                  /*Bit 4 : index of MAC address to use*/
        uint8_t  P2PMultipleInterface : 1;         /*Bit 5 : Use of multiple interface*/
        uint8_t  P2PListenProcedure : 1;           /*Bit 6 : P2P listen procedure*/
...
typedef struct __attribute__((__packed__)) WsmHiIeFlags_s {
        uint8_t  Beacon : 1;                       /*Update beacon*/ 
        uint8_t  ProbeResp : 1;                    /*Update probe response*/
        uint8_t  ProbeReq : 1;                     /*Update probe request*/
        uint8_t  Reserved1 : 5;                    /*Reserved*/      
        uint8_t  Reserved2;                        /*Reserved*/      
} WsmHiIeFlags_t;

typedef struct __attribute__((__packed__)) WsmHiMapLinkFlags_s {
        uint8_t  MapDirection : 1;                 /*0 to map, 1 to unmap type: Wsm*/
        uint8_t  Mfpc : 1;                         /*Set to 1 if STA advertised MFPC bit*/
        uint8_t  Reserved : 6;                     /*Reserved*/      
} WsmHiMapLinkFlags_t;

typedef struct __attribute__((__packed__)) WsmHiWepPairwiseKey_s {
        uint8_t  PeerAddress[WSM_API_PEER_ADDRESS_SIZE];   /*MAC address of the peer station type: SlConfigureIndStatus*/
        uint8_t  Reserved;                         /*Reserved type: SlConfigureSkeyInvld*/
        uint8_t  KeyLength;                        /*Key Length in bytes*/
        uint8_t  KeyData[WSM_API_KEY_DATA_SIZE];   /*Key data*/      
...
typedef struct __attribute__((__packed__)) WsmHiWepGroupKey_s {
        ...
        uint8_t  KeyLength;                        /*Key length in bytes*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*Reserved*/      
        uint8_t  KeyData[WSM_API_KEY_DATA_SIZE];   /*Key data*/      
} WsmHiWepGroupKey_t;

typedef struct __attribute__((__packed__)) WsmHiTkipPairwiseKey_s {
        uint8_t  PeerAddress[WSM_API_PEER_ADDRESS_SIZE];   /*MAC address of the peer station*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*Reserved*/      
...
typedef struct __attribute__((__packed__)) WsmHiTkipGroupKey_s {
        ...
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved*/      
...
typedef struct __attribute__((__packed__)) WsmHiMeasChannelLoadParams_s {
        uint8_t  Reserved;                         /*Reserved*/      
        uint8_t  ChannelLoadCCA;                   /*Determines whether to do a channel load (value 1) or a CCA measurement (value 0).*/
        uint16_t ChannelNum;                       /*Channel number on which the load needs to be measured.*/
        uint16_t RandomInterval;                   /*Upper bound of the random delay to be used before making the measurement. Expressed in units of TUs (for use with the channel load measurement).*/
        uint16_t MeasurementDuration;              /*Preferred or mandatory duration of the requested measurement. Expressed in units of TUs.*/
        uint64_t MeasurementStartTime;             /*Absolute time on which the measurement needs to be started (for use with the CCA measurement).*/
} WsmHiMeasChannelLoadParams_t;

typedef struct __attribute__((__packed__)) WsmHiMeasNoiseHistogramParams_s {
        uint8_t  Reserved;                         /*Reserved*/      
        uint8_t  IpiRpi;                           /*Indicates whether an IPI or RPI measurement is required.	0 - IPI measurement (start time determined by the RandomInterval) ; 1 - RPI measurement (start time determined by the MeasurementStartTime)*/
        uint16_t ChannelNum;                       /*Channel number on which the load needs to be measured.*/
        uint16_t RandomInterval;                   /*Upper bound of the random delay to be used before making the measurement. Expressed in units of TUs (for use with the IPI measurement).*/
        uint16_t MeasurementDuration;              /*Preferred or mandatory duration of the requested measurement. Expressed in units of TUs.*/
        uint64_t MeasurementStartTime;             /*Absolute time on which the measurement needs to be started (for use with the RPI measurement).*/
} WsmHiMeasNoiseHistogramParams_t;

typedef struct __attribute__((__packed__)) WsmHiMeasBeaconParams_s {
        uint16_t RandomInterval;                   /*Upper bound of the random delay to be used before making the measurement. Expressed in units of TUs (for use with the channel load measurement).*/
        uint16_t Reserved;                         /*Reserved*/      
        uint8_t  Band;                             /*The frequency band. 0 - 2.4GHz, 1 - 5GHz  == SAME AS WSM_HI_START_SCAN_REQ_BODY ==*/
        WsmHiScanType_t ScanType;                  /*Scan Type type: WsmHiScanType*/
        WsmHiScanFlags_t ScanFlags;                /*Scan Flags type: WsmHiScanFlags*/
        uint8_t  MaxTransmitRate;                  /*This parameter specifies the transmission rate to be used for sending probe requests.*/
        WsmHiAutoScanParam_t AutoScanParam;         /*Applicable to auto-scan ScanType only.         Interval period in TUs that the device will re-execute the requested scan. Maximum value supported by the device is 256 s. type: WsmHiAutoScanParam*/
        uint8_t  NumOfProbeRequests;               /*Number of probe requests (per SSID) sent to one (1) channel.         Zero (0) means that none is send, which means that a passive scan is to be done. Value greater than zero (0) means that an active scan is to be done.*/
        uint8_t  ProbeDelay;                       /*The delay time (in microseconds) period before sending a probe request.*/
        uint8_t  NumOfSSIDs;                       /*Number of SSIDs provided in the scan command (this is zero (0) in broadcast scan). The maximum number of SSIDs that the device can store is one.*/
        uint8_t  NumOfChannels;                    /*Number of channels to be scanned. [Maximum number is 1.]*/
        uint32_t MinChannelTime;                   /*time in TUs*/   
        uint32_t MaxChannelTime;                   /*time in TUs*/   
        int32_t  TxPowerLevel;                     /*time in TUs*/   
} WsmHiMeasBeaconParams_t;

typedef struct __attribute__((__packed__)) WsmHiMeasStaStatsParams_s {
        uint8_t  PeerMacAddress[WSM_API_PEER_MAC_ADDRESS_SIZE];   /*RA or TA MAC address for the frame statistics of this measurement 0*/
        uint16_t RandomInterval;                   /*Upper bound of the random delay to be used before making the measurement. Expressed in units of TUs.*/
        uint16_t MeasurementDuration;              /*Duration of the requested measurement in TUs.        Duration mandatory bit is not taken into account.        0 - Reports the current values for the requested statistics group data.        >0 - Reports the change in the requested statistics group data values measured within the measurement duration.*/
        uint8_t  GroupId;                          /*The requested statistics group.*/
        uint8_t  Reserved;                         /*Reserved*/      
} WsmHiMeasStaStatsParams_t;

typedef struct __attribute__((__packed__)) WsmHiMeasLinkMeasurementParams_s {
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_4];   /*Reserved 0*/    
} WsmHiMeasLinkMeasurementParams_t;

typedef struct __attribute__((__packed__)) WsmHiMeasChannelLoadResults_s {
        uint16_t Reserved;                         /*Reserved*/      
        uint8_t  ChannelLoadCCA;                   /*Determines whether to do a channel load (value 1) or a CCA measurement (value 0).*/
        uint8_t  ChannelNum;                       /*Channel number on which the load needs to be measured.*/
        uint64_t ActualMeasurementStartTime;       /*Value of the measuring STA TSF timer at the time the measurement has started.*/
        uint16_t MeasurementDuration;              /*Duration over which the channel-load report was measured. Expressed in units of TUs.*/
        uint8_t  CCAbusyFraction;                  /*Fractional duration over which CCA indicated that the channel was busy during the measurement duration.*/
        uint8_t  ChannelLoad;                      /*Proportion of the measurement duration for which the measuring STA determined the channel to be busy as indicated by either the physical (CCA) or the virtual carrier sense.*/
} WsmHiMeasChannelLoadResults_t;

typedef struct __attribute__((__packed__)) WsmHiMeasNoiseHistogramResults_s {
        uint16_t Reserved;                         /*Reserved*/      
        uint8_t  Reserved2;                        /*Reserved*/      
        uint8_t  ChannelNum;                       /*Channel number on which the histogram was measured.*/
        uint64_t ActualMeasurementStartTime;       /*Value of the measuring STA TSF timer at the time the measurement has started.*/
        uint16_t MeasurementDuration;              /*Duration over which the noise histogram report was measured. Expressed in units of TUs.*/
        uint8_t  AntennaID;                        /*Identifying number for the antenna(s) used for this measurement.*/
        uint8_t  IpiRpi;                           /*Indicates whether the result (of PI densities) is an IPI measurement result (value 0) or an RPI measurement result (value 1).*/
        uint8_t  Pi0Density;                       /*IPI/RPI density observed in the channel for level 0*/
        uint8_t  Pi1Density;                       /*IPI/RPI density observed in the channel for level 1*/
        uint8_t  Pi2Density;                       /*IPI/RPI density observed in the channel for level 2*/
        uint8_t  Pi3Density;                       /*IPI/RPI density observed in the channel for level 3*/
        uint8_t  Pi4Density;                       /*IPI/RPI density observed in the channel for level 4*/
        uint8_t  Pi5Density;                       /*IPI/RPI density observed in the channel for level 5*/
        uint8_t  Pi6Density;                       /*IPI/RPI density observed in the channel for level 6*/
        uint8_t  Pi7Density;                       /*IPI/RPI density observed in the channel for level 7*/
        uint8_t  Pi8Density;                       /*IPI density observed in the channel for level 8 (invalid in case of RPI measurement)*/
        uint8_t  Pi9Density;                       /*IPI density observed in the channel for level 9 (invalid in case of RPI measurement)*/
        uint8_t  Pi10Density;                      /*IPI density observed in the channel for level 10 (invalid in case of RPI measurement)*/
        uint8_t  Reserved3;                        /*Reserved*/      
} WsmHiMeasNoiseHistogramResults_t;

typedef struct __attribute__((__packed__)) WsmHiMeasBeaconResults_s {
        uint16_t MeasurementDuration;              /*Duration over which the beacon report was measured. Expressed in units of TUs.*/
        uint16_t Reserved;                         /*Reserved*/      
        uint64_t StartTsf;                         /*Value of the measuring STA TSF timer at the time the measurement started.*/
        uint64_t Duration;                         /*Measurement Duration*/
} WsmHiMeasBeaconResults_t;

typedef struct __attribute__((__packed__)) WsmHiMeasStaStatsResults_s {
        uint16_t MeasurementDuration;              /*The duration over which the change in statistics group data was measured and reported. Expressed in units of TUs.*/
        uint8_t  GroupId;                          /*Indicates the requested statistics group describing statistics group data. type: WsmHiStaStat*/
        uint8_t  StatisticsGroupDataLength;        /*Length (in octets) of statistics group data.*/
        uint8_t  StatisticsGroupData[WSM_API_STATISTICS_GROUP_DATA_SIZE];   /*Length (in octets) of statistics group data 0*/
} WsmHiMeasStaStatsResults_t;

typedef struct __attribute__((__packed__)) WsmHiMeasLinkMeasurementResults_s {
        int16_t  TransmitPower;                    /*Power level at which the remote device sent the link measurement request*/
        uint8_t  RxAntennaID;                      /*ID of the antenna on which the request was received*/
        uint8_t  TxAntennaID;                      /*ID of the antenna on which a reply was sent*/
        int32_t  NoiseLeveldBm;                    /*Noise level in dBm*/
        int8_t   LatestRssi;                       /*RSSI for the received link measurement request*/
        uint8_t  Reserved1;                        /*Reserved*/      
        uint8_t  Reserved2;                        /*Reserved*/      
        uint8_t  Reserved3;                        /*Reserved*/      
} WsmHiMeasLinkMeasurementResults_t;

typedef struct __attribute__((__packed__)) WsmHiP2PDeviceType_s {
        uint16_t CategoryId;                       /*The major category ID of the device.*/
        uint8_t  Oui12[WSM_API_OUI12_SIZE];        /*First two bytes of OUI 0*/
        uint8_t  Oui34[WSM_API_OUI34_SIZE];        /*Second part of OUI 0*/
        uint16_t SubCategoryId;                    /*Sub category ID of the device.*/
...
typedef struct __attribute__((__packed__)) WsmHiMibMacAddressDataFrameFilter_s {
        uint8_t  FilterMode;                       /*1 - if MAC Address matches Discard Frame. 2 - If MAC Address matches Accept Frame*/
        uint8_t  AddressType;                      /*1 - MAC address A1 will be used to be compared. 2 - MAC address A2 will be used to be compared. 3 - MAC address A3 will be used to be compared*/
        uint8_t  Address[WSM_API_ADDRESS_SIZE];    /*The MAC Address to filter on 0*/
} WsmHiMibMacAddressDataFrameFilter_t;

typedef struct __attribute__((__packed__)) WsmHiMibIpv4AddressDataFrameFilter_s {
        uint8_t  FilterMode;                       /*1 - if IPv4 Address matches Discard Frame. 2 - If IPv4 Address matches Accept Frame*/
        uint8_t  AddressMode;                      /*1 - Source address in IPv4 header. 2 - Destination address in IPv4 header*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*To ensure consistent padding 0*/
        uint8_t  Address[WSM_API_ADDRESS_SIZE_4];   /*The IPv4 address to filter on 0*/
} WsmHiMibIpv4AddressDataFrameFilter_t;

typedef struct __attribute__((__packed__)) WsmHiMibIpv6AddressDataFrameFilter_s {
        uint8_t  FilterMode;                       /*1 - if IPv6 Address matches Discard Frame. 2 - If IPv6 Address matches Accept Frame*/
        uint8_t  AddressMode;                      /*1 - Source address in IPv6 header. 2 - Destination address in IPv6 header*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*To ensure consistent padding 0*/
        uint8_t  Address[WSM_API_ADDRESS_SIZE_16];   /*The IPv6 address to filter on 0*/
} WsmHiMibIpv6AddressDataFrameFilter_t;

typedef struct __attribute__((__packed__)) WsmHiDebugIndEptaRtStats_s {
        uint32_t MsgStartIdentifier; 
        uint32_t IsBtRt;             
        uint32_t Timestamp;          
        uint32_t LinkId;             
        uint32_t NumRequests;        
        uint32_t NumGrants;          
        uint32_t ServiceWindow;      
        uint32_t ServiceInterval;    
        uint32_t BtTrafficPriority;  
        uint32_t BtTrafficType;      
        uint32_t IsBtMaster;         
} WsmHiDebugIndEptaRtStats_t;

typedef struct __attribute__((__packed__)) WsmHiDebugIndEptaNrtStats_s {
        uint32_t MsgStartIdentifier; 
        uint32_t IsBtRt;             
        uint32_t Timestamp;          
        uint32_t NumRequests;        
        uint32_t NumGrants;          
        uint32_t Reserved[WSM_API_RESERVED_SIZE_6];   /*To ensure consistent padding 0*/
} WsmHiDebugIndEptaNrtStats_t;

typedef struct __attribute__((__packed__)) WsmHiFilterConditionMask_s {
        uint8_t  MaskValue[WSM_API_MASK_VALUE_SIZE];
        uint8_t  ResultValue[WSM_API_RESULT_VALUE_SIZE];
} WsmHiFilterConditionMask_t;

typedef struct __attribute__((__packed__)) WsmHiMibEthertypeDataFrameFilter_s {
        uint8_t  FilterMode;                       /*Selects the filter mode type: WsmFilterMode*/
        uint8_t  Reserved;                         /*Reserved*/      
        uint16_t EtherType;                        /*EtherType to match.*/
} WsmHiMibEthertypeDataFrameFilter_t;

typedef struct __attribute__((__packed__)) WsmHiMibUdpportsDataFrameFilter_s {
        uint8_t  FilterMode;                       /*Selects the filter mode type: WsmFilterMode*/
        uint8_t  IsSrcPort;                        /*0 - Match against the destination port of the UDP header. 1 - Match against the source port of the UDP header*/
        uint16_t UDPPort;                          /*The UDP port number to filter on.*/
} WsmHiMibUdpportsDataFrameFilter_t;

typedef struct __attribute__((__packed__)) WsmHiMibMacAddresses_s {
        uint8_t  MacAddr0[WSM_API_MAC_ADDR0_SIZE];   /*mac address 0 character 0*/
        uint8_t  MacAddr1[WSM_API_MAC_ADDR1_SIZE];   /*mac address 1 character 0*/
} WsmHiMibMacAddresses_t;

typedef struct __attribute__((__packed__)) WsmHiMibMacAddr_s {
        uint8_t  MacAddr[WSM_API_MAC_ADDR_SIZE];   /*mac address character 0*/
} WsmHiMibMacAddr_t;

typedef struct __attribute__((__packed__)) WsmHiMibGrpAddrTable_s {
        uint32_t Enable;                           /*Address filtering configuration type: WsmEnable*/
        uint32_t NumOfAddresses;                   /*Number of addresses. A zero value clears the address table.*/
        WsmHiMibMacAddr_t AddressList[WSM_API_ADDRESS_LIST_SIZE];   /*Mac address 0 type: WsmHiMibMacAddr*/
...
typedef struct __attribute__((__packed__)) WsmHiMibWepDefaultKeyId_s {
        ...
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved 0*/    
} WsmHiMibWepDefaultKeyId_t;

typedef struct __attribute__((__packed__)) WsmHiMibArpIpAddrTable_s {
        uint8_t  ArpFilter : 1;                    /*Bit 0 = 1 - Enable ARP filtering*/
        uint8_t  ArpReply : 1;                     /*Bit 1 = 1 - Enable ARP reply*/
        uint8_t  Reserved : 6;                     /*Reserved*/      
        uint8_t  Reserved2[WSM_API_RESERVED2_SIZE];   /*Reserved2 0*/   
        uint8_t  Ipv4Address[WSM_API_IPV4_ADDRESS_SIZE];   /*The IP V4 address 0*/
} WsmHiMibArpIpAddrTable_t;

typedef struct __attribute__((__packed__)) WsmHiMibTemplateFrame_s {
        uint8_t  FrameType;                        /*Frame type type: WsmTmplt*/
        uint8_t  InitRate : 7;                     /*Bits 6 to 0 - Defines the initial transmission rate.*/
        uint8_t  Mode : 1;                         /*Bit 7 - Indicates the mode to use for 11n frames. 0 - Mixed ; 1 - Greenfield*/
        uint16_t FrameLength;                      /*The length of the frame in bytes. Measured from the first byte of the frame header to the last byte of the frame body (if any). The length is excluding the possible padding.*/
        uint8_t  Frame[WSM_API_FRAME_SIZE];        /*Frame (maximum of 1024 bytes)*/
...
typedef struct __attribute__((__packed__)) WsmHiMibBcnFilterTable_s {
        ...
        WsmHiIeTableEntry_t IeTable[WSM_API_IE_TABLE_SIZE];   /*IE-Table details. type: WsmHiIeTableEntry*/
...
typedef struct __attribute__((__packed__)) WsmHiMibRcpiRssiThreshold_s {
        ...
        uint8_t  Use : 1;                          /*Bit 1 = 0 - Use RCPI (default). 1 - Use RSSI*/
...
typedef struct __attribute__((__packed__)) WsmHiMibOverrideIntRate_s {
        ...
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*Reserved 0*/    
...
typedef struct __attribute__((__packed__)) WsmHiMibSetAssociationMode_s {
        ...
        uint8_t  PreambleType;                     /*Specifies the PLCP preamble type used type: WsmPreamble*/
        ...
        uint8_t  MpduStartSpacing;                 /*Defined according to IEEE 802.11 standard [1] Table 7-43k. The minimum MPDU start spacing subfield of A-MPDU parameters is as follows : type: WsmMpduStartSpacing*/
        uint32_t BasicRateSet;                     /*Contains the BasicRateSet to be used.*/
} WsmHiMibSetAssociationMode_t;

typedef struct __attribute__((__packed__)) WsmHiMibUpdateEptaConfigData_s {
        uint32_t EnablePta;                        /*0 - Disable (e)PTA. 1 - Enable (e)PTA. >=2 - Reserved*/
        uint32_t BluetoothQuality;                 /*This parameter sets the required Bluetooth quality that must be maintained. type: WsmBluetoothQuality*/
        uint32_t SubscribeBtEvent;                 /*0 - Unsubscribe from Bluetooth event indication. 1 - Subscribe to Bluetooth event indication. >=2 - Reserved*/
        uint32_t PtaDebugCommand;                  /*To be defined*/ 
...
typedef struct __attribute__((__packed__)) WsmHiMibSetHostMsgTypeFilter_s {
        uint16_t FrameCtrlFilterMask;              /*The mask to be applied to the frame control field of the receive indication frame.*/
        uint16_t FrameCtrlFilterValue;             /*The value to be compared to the masked frame control field value.             If the values match, then a message type of 1 is passed to the host with the Rx indication message, otherwise, a value of 0 applies.*/
} WsmHiMibSetHostMsgTypeFilter_t;

typedef struct __attribute__((__packed__)) WsmHiMibP2PFindInfo_s {
        uint8_t  Band;                             /*Band*/          
        uint8_t  Reserved;                         /*Reserved*/      
        uint16_t ListenChannel;                    /*The channel on which the P2P device should perform listen procedures.*/
        uint8_t  P2PMinDiscoverableTime;           /*The minimum time [in units of 100TU s] to be spent on a given channel during the listen state.*/
        uint8_t  P2PMaxDiscoverableTime;           /*The maximum time [in units of 100TU s] to be spent on a given channel during the listen state.*/
        uint8_t  MaxTransmitRate;                  /*The maximum rate at which probe request should be transmitted.*/
        uint8_t  ProbeDelay;                       /*Delay before transmitting a probe frame.*/
        uint16_t FindTimeOut;                      /*The total time (in units of 100TU s) that device should perform P2P find procedures. The maximum timeout that can be set is 0xFFFE. The value 0xFFFF indicates no timeout.*/
        uint8_t  NumProbs;                         /*Number of probe requests to be transmitted on each channel during the search phase.*/
        uint8_t  NumChannels;                      /*Number of channels to be searched.*/
        /* WsmHiChannelDef_t SocialChannels;      */   /*Find the definition of type channels as part of Start-Scan API. type: WsmHiChannelDef*/
} WsmHiMibP2PFindInfo_t;

typedef struct __attribute__((__packed__)) WsmHiMibP2PPsModeInfo_s {
        uint8_t  CtWin : 7;                        /*Bits 0 to 6: - CTWindow in units of TU*/
        uint8_t  Opportunistic : 1;                /*Bit 7: 1 -> Opportunistic PS mode enable ; 0 -> Opportunistic PS mode disable*/
        uint8_t  Count;                            /*Indicates the number of absent intervals. 0 - Do not use NoA or cancel existing schedule. 255 - Continues schedule*/
        uint8_t  Reserved;                         /*Reserved for Write operation. Read Operation will provide NoAIndex at the point of read operation.*/
        uint8_t  DtimCount;                        /*Number of DTIM intervals to wait before starting the NoA Schedule*/
        uint32_t Duration;                         /*Maximum duration in microseconds, the P2P GO can remain absent following the start of a NoA interval.*/
        uint32_t Interval;                         /*Length of NoA interval in microseconds*/
        uint32_t StartTime;                        /*GO: Time offset in microseconds to the start of the NoA schedule. This is the time offset from the last DTIM beacon before NoA schedule starts.            Client: The start time for the schedule expressed in terms of the lower 4 bytes of the TSF timer.*/
} WsmHiMibP2PPsModeInfo_t;

typedef struct __attribute__((__packed__)) WsmHiMibEtherTypeDataFrameFilterSet_s {
        uint8_t  NrFilters;                        /*The number of filters defined by the host. A maximum of four filters is supported. (WSM_MAX_FILTER_ELEMENTS).*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved 0*/    
...
typedef struct __attribute__((__packed__)) WsmHiMibUdpPortDataFrameFilterSet_s {
        uint8_t  NrFilters;                        /*The number of filters defined by the host. A maximum of four filters is supported. (WSM_MAX_FILTER_ELEMENTS).*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved 0*/    
...
typedef struct __attribute__((__packed__)) WsmHiMibMagicDataFrameFilter_s {
        uint8_t  FilterMode;                       /*Selects the filter mode type: WsmFilterMode*/
        ...
        uint8_t  Reserved;                         /*Reserved*/      
        uint8_t  MagicPattern[WSM_API_MAGIC_PATTERN_SIZE];   /*The magic byte pattern to match 0*/
} WsmHiMibMagicDataFrameFilter_t;

typedef struct __attribute__((__packed__)) WsmHiMibBlockAckInfo_s {
        uint8_t  BufferSize;         
...
typedef struct __attribute__((__packed__)) WsmHiMibDisableBssidFilter_s {
        uint8_t  Filter;                           /*0: enable BSSID filtering. this is the default value. 1: disable BSSID filtering.*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved 0*/    
...
typedef struct __attribute__((__packed__)) WsmHiMibArpKeepAlivePeriod_s {
        ...
        uint8_t  SenderIpv4Address[WSM_API_SENDER_IPV4_ADDRESS_SIZE];   /*IP address of sender 0*/
        uint8_t  TargetIpv4Address[WSM_API_TARGET_IPV4_ADDRESS_SIZE];   /*IP address of target 0*/
} WsmHiMibArpKeepAlivePeriod_t;

typedef struct __attribute__((__packed__)) WsmHiMibPseudoRate_s {
        int16_t  PseudoRatePowerStep;              /*TX Power Step in Q4.4 format (4 decimal places).*/
        uint8_t  PseudoRateReference;              /*The reference rate that will be used to build the power table. This is normally the highest rate that will be employed and should be common to all peers.*/
        uint8_t  Reserved;                         /*Reserved*/      
} WsmHiMibPseudoRate_t;

typedef struct __attribute__((__packed__)) WsmHiMibMacAddressDataFrameFilterSet_s {
        uint8_t  NrFilters;                        /*The number of filters defined in the message by the host. A maximum of eight filters is supported.*/
        uint8_t  ActionMode;                       /*General Enable/Disable flag for all MAC addresses. 0 - Disable MAC address filtering. 1 - Enable MAC address filtering*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*To ensure consistent byte padding. 0*/
...
typedef struct __attribute__((__packed__)) WsmHiMibIpv4AddressDataFrameFilterSet_s {
        uint8_t  NrFilters;                        /*The number of filters defined in the message by the host. A maximum of eight filters is supported.*/
        uint8_t  ActionMode;                       /*0 - Disabled IPv4 Address filtering. 1 - Enabled IPV4 Address filtering*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*To ensure consistent byte padding 0*/
...
typedef struct __attribute__((__packed__)) WsmHiMibIpv6AddressDataFrameFilterSet_s {
        uint8_t  NrFilters;                        /*The number of filters defined in the message by the host. A maximum of eight filters is supported.*/
        uint8_t  ActionMode;                       /*0 - Disabled IPv6 Address filtering. 1 - Enabled IPV6 Address filtering*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*To ensure consistent byte padding 0*/
...
typedef struct __attribute__((__packed__)) WsmHiMibForwardingOffloadSet_s {
        uint8_t  FwEnable;           
        uint8_t  FwOffloadFlags;     
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_2];   /*Reserved 0*/    
...
typedef struct __attribute__((__packed__)) WsmHiMibP2PDeviceInfo_s {
        WsmHiP2PDeviceType_t PrimaryDevice;         /*The major category ID of the device. type: WsmHiP2PDeviceType*/
        uint8_t  Reserved1[WSM_API_RESERVED1_SIZE];   /*Padding 0*/     
        uint8_t  DevNameSize;                      /*Length of the local device name. Maximum value is 32.*/
        uint8_t  LocalDevName[WSM_API_LOCAL_DEV_NAME_SIZE];   /*Friendly name of the device 0*/
        uint8_t  Reserved2[WSM_API_RESERVED2_SIZE];   /*Padding 0*/     
        uint8_t  NumSecDevSupported;               /*Number of secondary device types supported.*/
        /* WsmHiP2PDeviceType_t SecondaryDeviceTypeList[WSM_API_SECONDARY_DEVICE_TYPE_LIST_SIZE]; */   /*Secondary device type information. Maximum supported value for N is 16.*/
} WsmHiMibP2PDeviceInfo_t;

typedef struct __attribute__((__packed__)) WsmHiMibWcdmaBand_s {
        uint8_t  WCDMABand;                        /* type: WsmWcdmaBand*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved 0*/    
} WsmHiMibWcdmaBand_t;
...
typedef union filter_condition_u {
        uint8_t                                       asValue[WSM_API_ASVALUE_SIZE];   /* Element :0*/   
        WsmHiFilterConditionMask_t                    asMask;                         /* Element :1*/   
} FilterCondition_t;

typedef union wsm_event_data_u {
        uint32_t                                      ErrorStatus;                    /* Element :0*/   
        uint8_t                                       RcpiRssi;                       /* Element :4*/   
        uint32_t                                      Bt_Inactive;                    /* Element :5*/   
        uint32_t                                      BtActive;                       /* Element :6*/   
        uint32_t                                      P_S_Mode_Error;                 /* Element :7*/   
        uint32_t                                      P_A_S_Event;                    /* Element :8*/   
} WsmEventData_t;
...
typedef union wsm_hi_debug_ind__debug_data_u {
        ...
        uint32_t                                      RawData[WSM_API_RAWDATA_SIZE];   /* Element :2*/   
...
typedef union wsm_mib_data_u {
        ...
        uint32_t                                      IbssPsConfig;                   /* Element :4107*/
        ...
        uint32_t                                      SelectCcaMethod;                /* Element :4114*/
        WsmHiMibSetUapsdInformation_t                 SetUapsdInformation;            /* Element :4115*/
        uint32_t                                      SetAutoCalibrationMode;         /* Element :4117*/
        ...
        WsmHiMibP2PDeviceInfo_t                       P2PDeviceInfo;                  /* Element :4125*/
        WsmHiMibWcdmaBand_t                           WCDMA_Band;                     /* Element :4126*/
        WsmHiMibGroupSeqCounter_t                     GroupSeqCounter;                /* Element :4127*/
        uint32_t                                      ProtectedMgmtFramesPolicy;      /* Element :4128*/
        uint32_t                                      SetHtProtection;                /* Element :4129*/
        WsmHiMibTsfCounter_t                          TSFCounter;                     /* Element :4131*/
        uint32_t                                      UseMultiTxConfMsg;              /* Element :4132*/
...
typedef struct __attribute__((__packed__)) WsmHiGenericCnf_s {
        HiMsgHdr_t Header;
        uint32_t    Status;
} WsmHiGenericCnf_t ;
...
typedef struct __attribute__((__packed__)) WsmHiResetCnfBody_s {
        uint32_t Status;                           /*Status to indicate the status of the request.*/
} WsmHiResetCnfBody_t;
...
typedef struct __attribute__((__packed__)) WsmHiReadMibReqBody_s {
        uint16_t MibId;                            /*ID of the MIB to be read.*/
        uint16_t Reserved;                         /*Reserved*/      
...
typedef struct __attribute__((__packed__)) WsmHiReadMibCnfBody_s {
        uint32_t Status;                           /*Status to indicate the status of the request. If WSM_STATUS_SUCCESS is returned, MIB data will follow.*/
        uint16_t MibId;                            /*ID of the MIB to be read.*/
...
typedef struct __attribute__((__packed__)) WsmHiStartScanReqBody_s {
        uint8_t  Band;                             /*The radio band. 0 - 2.4 GHz band ; 1 - 5 GHz band*/
        ...
        uint8_t  NumOfProbeRequests;               /*Number of probe requests (per SSID) sent to one (1) channel.     Zero (0) means that none is send, which means that a passive scan is to be done. Value greater than zero (0) means that an active scan is to be done.*/
        uint8_t  ProbeDelay;                       /*The delay time (in microseconds) period before sending a probe request.*/
        uint8_t  NumOfSSIDs;                       /*Number of SSIDs provided in the scan command (this is zero (0) in broadcast scan). The maximum number of SSIDs that the device can store is two.*/
        uint8_t  NumOfChannels;                    /*Number of channels to be scanned. [Maximum number is 14.]*/
        uint32_t MinChannelTime;                   /*time in TUs*/   
        uint32_t MaxChannelTime;                   /*time in TUs*/   
        int32_t  TxPowerLevel;                     /*in 0.1dBm unit*/
        /* WsmHiSsidDef_t SsidDef[WSM_API_SSID_DEF_SIZE]; */
        /* uint8_t  ChannelList[WSM_API_CHANNEL_LIST_SIZE]; */   /*Optional if the Bit 24 in the Flags field is set.*/
...
typedef struct __attribute__((__packed__)) WsmHiStartScanCnfBody_s {
        uint32_t Status;                           /*Status to indicate the status of the request.     If WSM_STATUS_SUCCESS is not returned, the scan process will not start. No scan-completion indication will be sent to the host.*/
...
typedef struct __attribute__((__packed__)) WsmHiStopScanCnfBody_s {
        uint32_t Status;                           /*Error codes for the stop-scan request.     The stop-scan request acceptance has no impact on whether the scan completion is sent.*/
...
typedef struct __attribute__((__packed__)) WsmHiScanCmplIndBody_s {
        uint32_t Status;                           /*Result of the scanning process.*/
        uint8_t  PmMode;                           /*The current 802.11 power management mode of the WLAN device.*/
...
typedef struct __attribute__((__packed__)) WsmHiTxReqBody_s {
        uint32_t PacketId;                         /*Packet identifier that is meant to be used in completion.*/
        uint8_t  MaxTxRate;                        /*Maximum transmit rate.*/
        WsmHiQueueId_t QueueId;            
        uint8_t  More;                             /*True: another packet is pending on the host for transmission.*/
        WsmHiTxFlags_t TxFlags;            
        uint32_t Reserved;                         /*Reserved*/      
        uint32_t ExpireTime;                       /*The elapsed time in TUs, after the initial transmission of an MSDU, after which further attempts to transmit the MSDU will be terminated.         Overrides the global dot11MaxTransmitMsduLifeTime setting [optional].        The device will set the default value if this is 0.*/
        WsmHiHtTxParameters_t HtTxParameters;     
        uint32_t Frame;                            /*An 802.11 frame.*/
} WsmHiTxReqBody_t;
...
typedef struct __attribute__((__packed__)) WsmHiTxCnfBody_s {
        ...
        uint32_t Status;                           /*Transmit complete status.*/
        uint8_t  TxedRate;                         /*The data rate at which the frame was successfully transmitted.*/
        uint8_t  AckFailures;                      /*The number of times the frame was transmitted without receiving an acknowledgement.*/
...
typedef struct __attribute__((__packed__)) WsmHiRxIndBody_s {
        uint32_t Status;                           /*Result of the packet receive process.*/
        uint16_t ChannelNumber;                    /*Specifies the channel of the received packet.*/
        uint8_t  RxedRate;                         /*The data rate at which the frame was successfully received.*/
        uint8_t  RcpiRssi;                         /*This value is expressed in dBm as signed Q8.0 format for RSSI and unsigned Q7.1 format for RCPI.     The value type of RCPI or RSSI is determined by RssiRcpiMode bit 1. The default is RCPI type.*/
        WsmHiRxFlags_t RxFlags;            
        uint32_t Frame;                            /*Result of the packet receive process.*/
        /* uint8_t  Timestamp[WSM_API_TIMESTAMP_SIZE]; */   /*Optional if the Bit 24 in the Flags field is set.*/
} WsmHiRxIndBody_t;
...
typedef struct __attribute__((__packed__)) WsmHiJoinReqBody_s {
        uint8_t  Mode;                             /*Specifies the operation mode of the station. For the P2P group negotiation, the BSS mode should be used.*/
        uint8_t  Band;                             /*The frequency band. 0 - 2.4GHz, 1 - 5GHz*/
        ...
        uint8_t  PreambleType;                     /*The frequency band.*/
        uint8_t  ProbeForJoin;                     /*Specifies if a probe request should be send with the specified SSID when joining the network.    This option is to acquire the TSF time of the BSS or IBSS that is to be joined as fast as possible. The TSF time is absorbed from the probe response or from beacon, whichever is received first.*/
        uint8_t  DTIMPeriod;                       /*Reserved*/      
        ...
        uint32_t BasicRateSet;                     /*A bit mask that defines the BSS basic rate set.*/
} WsmHiJoinReqBody_t;
...
typedef struct __attribute__((__packed__)) WsmHiJoinCnfBody_s {
        uint32_t Status;                           /*Error code for the join operation.*/
        int32_t  MinPowerLevel;                    /*Minimum transmission power level in units of 0.1 dBm*/
        int32_t  MaxPowerLevel;                    /*Maximum transmission power level in units of 0.1 dBm*/
} WsmHiJoinCnfBody_t;
...
typedef struct __attribute__((__packed__)) WsmHiSetPmModeCmplIndBody_s {
        ...
        uint8_t  PmMode;                           /*Current WLAN device PM mode state*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved*/      
...
typedef struct __attribute__((__packed__)) WsmHiSetBssParamsReqBody_s {
        ...
        uint8_t  BeaconLostCount;                  /*The number of lost consecutive beacons after which the WLAN device should indicate the BSS-Lost event to the WLAN host driver.             Value of 0 disables the BSS-Lost event indications, thereby also disabling the BSS-gained event indications.*/
        uint16_t AID;                              /*Specifies the AID received during the association process.*/
        uint32_t OperationalRateSet;               /*The operational rate set mask defined in terms of a bit map corresponding to the rate indices. This will be used for modem enabling purposes.*/
...
typedef struct __attribute__((__packed__)) WsmHiSetBssParamsCnfBody_s {
        uint32_t Status;                           /*Error codes for the Set-BssParams request.*/
...
typedef struct __attribute__((__packed__)) WsmHiAddKeyReqBody_s {
        uint8_t  Type;                             /*Type of the key to be added:*/
...
typedef struct __attribute__((__packed__)) WsmHiTxQueueParamsReqBody_s {
        uint8_t  QueueId;                          /*Specifies the Access Category Index (ACI) of the queue*/
        uint8_t  Reserved2;                        /*Reserved*/      
        uint8_t  AckPolicy;                        /*ACK frame policy of the specified queue*/
        uint8_t  Reserved1;                        /*Reserved*/      
        uint32_t MaxTransmitLifetime;              /*This attribute will be the elapsed time in TUs, after the initial transmission of an MSDU,     after which further attempts to transmit the MSDU will be terminated. This is a standard 802.11 MIB variable. If a value of 0 is specified, the default value is used.*/
...
typedef struct __attribute__((__packed__)) WsmHiTxQueueParamsCnfBody_s {
        uint32_t Status;                           /*Error codes for the Set-TxQueueParams request.*/
...
typedef struct __attribute__((__packed__)) WsmHiEdcaParamsReqBody_s {
        uint16_t CwMin[WSM_API_CW_MIN_SIZE];       /*CwMin (in slots) for the access class. Indices map to access category priorities as follows :    0 - AC_BK (background), 1 - AC_BE (best effort), 2 - AC_VI (video), 3 - AC_VO (voice)*/
        uint16_t CwMax[WSM_API_CW_MAX_SIZE];       /*CwMax (in slots) for the access class.*/
        uint8_t  AIFSN[WSM_API_AIFSN_SIZE];        /*AIFS (in slots) for the access class.*/
        uint16_t TxOpLimit[WSM_API_TX_OP_LIMIT_SIZE];   /*TX OP limit (in microseconds) for the access class.*/
        uint32_t MaxReceiveLifetime[WSM_API_MAX_RECEIVE_LIFETIME_SIZE];   /*Range of 1 to 0xFFFF FFFF. This is a standard 802.11 MIB variable. If a value of 0 is specified, the default value is used.*/
...
typedef struct __attribute__((__packed__)) WsmHiSwitchChannelReqBody_s {
        ...
        uint8_t  ChannelSwitchCount;               /*Number of TBTTs until the channel switch occurs. 0 - At any time. 1 - Immediately before the next TBTT.*/
        uint16_t NewChannelNumber;                 /*The new channel number to switch to.*/
        WsmHiSwitchFlags_t SwitchFlags;        
        uint8_t  Band;                             /*The band for the new channel. 0 - 2.4 GHz band, 1 - 5 GHz band*/
        uint8_t  NumTxSlotsReserved;               /*The number of TX slots reserved on the device for frame transmission in the new channel.             At the start of channel switch, excessive TX requests queued for the original channel will be re-queued to the host.*/
        uint32_t BasicRateSet;                     /*The basic rate set (in bit mask) for the new channel.*/
        uint16_t DurationInKus;                    /*The duration (in Kus) in which the new channel will be switched in.*/
        uint16_t ForeignBssDtimPeriodInKus;        /*Foreign BSS DTIM period in Kus. This field is used foreign BSS synchronization.*/
        uint32_t ForeignBssTsfDiffLow;             /*The low 32-bit word of TSF difference between the foreign TSF and our TSF.*/
        uint32_t ForeignBssTsfDiffHigh;            /*The high 32-bit word of TSF difference between the foreign TSF and our TSF.*/
...
typedef struct __attribute__((__packed__)) WsmHiSwitchChannelCnfBody_s {
        uint32_t Status;                           /*Error codes for the switch-channel request.*/
...
typedef struct __attribute__((__packed__)) WsmHiSwitchChannelIndBody_s {
        uint32_t State;                            /*The state code for the switch-channel operation:*/
...
typedef struct __attribute__((__packed__)) WsmHiStartReqBody_s {
        WsmHiMode_t Mode;               
        uint8_t  Band;                             /*The frequency band. 0 - 2.4 GHz band, 1 - 5 GHz band*/
        uint16_t ChannelNumber;                    /*Channel number*/
        uint32_t CTWindow;                         /*Client traffic window in units of TU.\ Valid only when mode = P2P_GO.\ This value should be less than the beacon interval.*/
        ...
        uint8_t  PreambleType;                     /*Specifies the PLCP preamble type used:*/
        uint8_t  ProbeDelay;                       /*The delay time (in microseconds) period before sending a probe-request.*/
        uint8_t  SsidLength;                       /*Length of the SSID*/
        uint8_t  Ssid[WSM_API_SSID_SIZE];          /*SSID of the BSS or P2P_GO to be started now.*/
        uint32_t BasicRateSet;                     /*The basic supported rates for the MiniAP.*/
} WsmHiStartReqBody_t;
...
typedef struct __attribute__((__packed__)) WsmHiStartCnfBody_s {
        uint32_t Status;                           /*Status code for the start confirmation*/
} WsmHiStartCnfBody_t;
...
typedef struct __attribute__((__packed__)) WsmHiBeaconTransmitReqBody_s {
        uint8_t  EnableBeaconing;                  /*1 to start beacon transmission, 0 to stop.*/
        uint8_t  Reserved[WSM_API_RESERVED_SIZE_3];   /*Reserved*/      
...
typedef struct __attribute__((__packed__)) WsmHiSuspendResumeTxIndBody_s {
        ...
        uint16_t TxResumeFlagsPerIf[WSM_API_TX_RESUME_FLAGS_PER_IF_SIZE];   /*Set to 0*/      
...
typedef struct __attribute__((__packed__)) WsmHiUpdateIeCnfBody_s {
        uint32_t Status;                           /*Status of the Update-IE request*/
...
typedef struct __attribute__((__packed__)) WsmHiMapLinkCnfBody_s {
        uint32_t Status;                           /*Status of the Map-Link request*/
...
typedef struct __attribute__((__packed__)) WsmHiMultiTransmitCnfBody_s {
        ...
        uint32_t TxConfPayload;                    /*Variable length*/
...
typedef struct __attribute__((__packed__)) WsmHiDebugIndBody_s {
        uint32_t DbgId;                            /*Identifies the indication*/
...
typedef struct __attribute__((__packed__)) WsmHiBaTimeoutIndBody_s {
        uint8_t  TID;                
        uint8_t  Reserved;           
        uint8_t  TransmitAddress[WSM_API_TRANSMIT_ADDRESS_SIZE];
