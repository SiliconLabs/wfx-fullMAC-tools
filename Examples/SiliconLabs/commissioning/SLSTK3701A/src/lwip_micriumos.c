...
static void lwip_app_wifi_config_prompt (void)
{
  ...
  console_get_input_tmo(buf, 1 /*Enter*/ + 1 /*NUL*/, 5, 1);
  ...
  if (buf[0] == '\r' || buf[0] == '\n') {
    ...
    if (buf[0] == '1') {
      ...
      console_get_input(softap_ssid, sizeof(softap_ssid), 1);

      do {
        error = false;
        printf("Enter the Passkey of the SoftAP you want to create (8-chars min):\n");
        console_get_input(softap_passkey, sizeof(softap_passkey), 1);

        if (strlen(softap_passkey) < 8) {
          printf("Size error\n");
          error = true;
        }
      } while (error);
