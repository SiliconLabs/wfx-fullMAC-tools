...
extern osThreadId secureLinkTaskHandle;
extern osSemaphoreId s_xSLSemaphore;
                                       ...
                                       uint8_t *sl_host_pub_key)
{
  ...
  if(xSemaphoreTake(s_xSLSemaphore, portMAX_DELAY) != pdTRUE)
  {
    return SL_STATUS_WIFI_SECURE_LINK_EXCHANGE_FAILED;
  }
...
error_handler:
  if (xSemaphoreGive(s_xSLSemaphore) != pdTRUE)
  {
...
sl_status_t sl_wfx_host_schedule_secure_link_renegotiation(void)
{
  ...
  xTaskNotifyGive( secureLinkTaskHandle );
  return SL_STATUS_OK;
}
