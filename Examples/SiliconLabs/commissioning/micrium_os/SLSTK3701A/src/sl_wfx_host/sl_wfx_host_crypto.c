...
extern OS_MUTEX   sl_wfx_securelink_rx_mutex;
extern OS_TCB     sl_wfx_securelink_task_tcb;
...
sl_status_t sl_wfx_host_verify_pub_key (sl_wfx_securelink_exchange_pub_keys_ind_t* response_packet,
  ...
  OSMutexPend (&sl_wfx_securelink_rx_mutex,0,OS_OPT_PEND_BLOCKING,0,&err);
...
error_handler:
  OSMutexPost(&sl_wfx_securelink_rx_mutex,OS_OPT_POST_NONE,&err);
  if (RTOS_ERR_CODE_GET(err) != RTOS_ERR_NONE) {
                                                 ...
                                                 uint8_t* session_key)
{
  ...
  /* Set the crypto key*/
  crypto_status = mbedtls_ccm_setkey (&ccm_context,
		                              MBEDTLS_CIPHER_ID_AES,
		                              session_key,
									  SL_WFX_SECURE_LINK_SESSION_KEY_BIT_COUNT);
...
error_handler:
  ...
  OSMutexPost(&sl_wfx_securelink_rx_mutex, OS_OPT_POST_NONE,&err);
  if (RTOS_ERR_CODE_GET(err) != RTOS_ERR_NONE) {
    printf ("ERROR: sl_wfx_securelink_mutex. unable to post.\n");
  }
  return status;
}
...
sl_status_t sl_wfx_host_schedule_secure_link_renegotiation (void) {
  ...
  RTOS_ERR err;
  OSTaskSemPost(&sl_wfx_securelink_task_tcb,OS_OPT_POST_NONE, &err);
  return SL_STATUS_OK;
}
/**************************************************************************//**
 * Reverse bytes
