...
extern OS_MUTEX   wf200_securelink_rx_mutex;
extern OS_TCB     WF200SecurelinkTaskTCB;
...
{
  sl_status_t result = SL_WIFI_SECURE_LINK_MAC_KEY_ERROR;
...
sl_status_t sl_wfx_host_compute_pub_key( sl_wfx_securelink_exchange_pub_keys_req_body_t* request, const uint8_t* sl_mac_key )
{
  ...
  status =  mbedtls_md_setup( &host_message_digest_context, mbedtls_md_info_from_type( md_type ), 1 ); //use hmac
  status += mbedtls_md_hmac_starts( &host_message_digest_context, sl_mac_key, SL_WFX_HOST_PUB_KEY_SIZE );
  status += mbedtls_md_hmac_update( &host_message_digest_context, request->host_pub_key, SL_WFX_HOST_PUB_KEY_SIZE );
  status += mbedtls_md_hmac_finish( &host_message_digest_context, request->host_pub_key_mac );
  SL_WFX_ERROR_CHECK( status );

error_handler:
  if(status != SL_SUCCESS)
  {
	  return SL_WIFI_SECURE_LINK_EXCHANGE_FAILED;
  }
  return status;
}
                                        ...
                                        const uint8_t* sl_mac_key)
{
  RTOS_ERR err;
  sl_status_t status = SL_SUCCESS;
  uint8_t shared_key_digest[SL_WFX_HOST_PUB_KEY_SIZE];

  OSMutexPend (&wf200_securelink_rx_mutex,0,OS_OPT_PEND_BLOCKING,0,&err);
  ...
  mbedtls_md_context_t ineo_message_digest_context;

  mbedtls_md_init( &ineo_message_digest_context );
  mbedtls_md_setup( &ineo_message_digest_context, mbedtls_md_info_from_type( md_type ), 1 ); //use hmac
  mbedtls_md_hmac_starts( &ineo_message_digest_context, sl_mac_key, SL_WFX_NCP_PUB_KEY_SIZE );
  mbedtls_md_hmac_update( &ineo_message_digest_context, response_packet->body.ncp_pub_key, SL_WFX_NCP_PUB_KEY_SIZE );
  mbedtls_md_hmac_finish( &ineo_message_digest_context, temp_key_location );
  ...
  {
    status = SL_WIFI_SECURE_LINK_EXCHANGE_FAILED;
    goto error_handler;
  }
...
error_handler:
  ...
  OSMutexPost(&wf200_securelink_rx_mutex,OS_OPT_POST_NONE,&err);
  if (RTOS_ERR_CODE_GET(err) != RTOS_ERR_NONE)
  {
    printf ("ERROR: wf200_securelink_rx_mutex. unable to post.\n");
  }
  return status;
}
                                                 ...
                                                 uint8_t* session_key)
{
  ...
  sl_status_t status = SL_ERROR_OUT_OF_HEAP;
  int crypto_status;
  ...
  // Decrypt the data
  status = mbedtls_ccm_auth_decrypt( &ccm_context, length,
...
error_handler:
  ...
  if (RTOS_ERR_CODE_GET(err) != RTOS_ERR_NONE)
  {
    printf ("ERROR: wf200_securelink_mutex. unable to post.\n");
  }
  return status;
}
...
sl_status_t sl_wfx_host_schedule_secure_link_renegotiation(void)
{
  ...
  RTOS_ERR err;
  OSTaskSemPost (&WF200SecurelinkTaskTCB,OS_OPT_POST_NONE, &err);
  return SL_SUCCESS;
}
