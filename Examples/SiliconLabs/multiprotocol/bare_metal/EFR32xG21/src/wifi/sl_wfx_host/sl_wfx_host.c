...
#else
#error "WFX200 EXP board not specified or not supported"
#endif
...
void sl_wfx_ap_client_connected_callback(uint8_t* mac);
...
sl_status_t sl_wfx_host_post_event(sl_wfx_generic_message_t *event_payload)
{
  switch (event_payload->header.id) {
    ...
    case SL_WFX_AP_CLIENT_CONNECTED_IND_ID:
    {
      ...
      sl_wfx_ap_client_connected_callback(client_connected_indication->body.mac);
      break;
    }
...
void sl_wfx_scan_result_callback(sl_wfx_scan_result_ind_body_t* scan_result)
{
  ...
  if (scan_count <= SL_WFX_MAX_SCAN_RESULTS) {
    scan_list[scan_count_saved].ssid_def = scan_result->ssid_def;
    scan_list[scan_count_saved].channel = scan_result->channel;
    scan_list[scan_count_saved].security_mode = scan_result->security_mode;
    scan_list[scan_count_saved].rcpi = scan_result->rcpi;
    memcpy(scan_list[scan_count_saved].mac, scan_result->mac, 6);
    scan_count_saved++;
  }
}
