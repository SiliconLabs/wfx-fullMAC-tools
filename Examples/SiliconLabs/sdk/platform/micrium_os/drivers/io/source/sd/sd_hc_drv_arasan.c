                            ...
                            RTOS_ERR    *p_err)
{
  ...
  DEF_BIT_CLR(p_regs->PWRCTRL, SDHC_PWRCTRL_SD_BUS_PWR_ON);
                                  ...
                                  RTOS_ERR    *p_err)
{
  ...
  if (freq == 0) {
    ...
    DEF_BIT_CLR(p_regs->CLKCTRL, SDHC_CLKCTRL_SD_CLK_EN);
    DEF_BIT_CLR(p_regs->CLKCTRL, SDHC_CLKCTRL_INT_CLK_STBL);
    DEF_BIT_CLR(p_regs->CLKCTRL, SDHC_CLKCTRL_INT_CLK_EN);
    return;
  }
  ...
  //                                                               Sets Prescale Value.
  DEF_BIT_FIELD_WR(clkctrl, clk_prescaler, SDHC_CLKCTRL_SDCLK_FREQ_SEL);
                                        ...
                                        RTOS_ERR         *p_err)
{
  ...
  switch (volt) {
    case SD_CARD_BUS_VOLT_3_3:
      DEF_BIT_FIELD_WR(p_regs->PWRCTRL, SDHC_PWRCTRL_SD_BUS_3_3V, SDHC_PWRCTRL_SD_BUS_VOLT_MSK);
      break;

    case SD_CARD_BUS_VOLT_3:
      DEF_BIT_FIELD_WR(p_regs->PWRCTRL, SDHC_PWRCTRL_SD_BUS_3V, SDHC_PWRCTRL_SD_BUS_VOLT_MSK);
      break;

    case SD_CARD_BUS_VOLT_1_8:
      DEF_BIT_FIELD_WR(p_regs->PWRCTRL, SDHC_PWRCTRL_SD_BUS_1_8V, SDHC_PWRCTRL_SD_BUS_VOLT_MSK);
      break;
                                        ...
                                        RTOS_ERR                *p_err)
{
  ...
  switch (volt) {
    case SD_CARD_BUS_SIGNAL_VOLT_3_3:
      DEF_BIT_CLR(p_regs->HOSTCTRL2, SDHC_HOSTCTRL2_1_8V_SIG_EN);
      break;
                                   ...
                                   RTOS_ERR    *p_err)
{
  ...
  DEF_BIT_CLR(p_regs->HOSTCTRL1, SDHC_HOSTCTRL1_EXT_DATA_XFER_8B);
  DEF_BIT_CLR(p_regs->HOSTCTRL1, SDHC_HOSTCTRL1_DATAXFERWIDTH_4BIT);
                                ...
                                RTOS_ERR    *p_err)
{
  ...
  if (DEF_BIT_IS_SET(p_cmd->Flags, SD_CARD_CMD_FLAG_DATA_ABORT)) {
    DEF_BIT_FIELD_WR(cmd, SDHC_CMD_CMDTYPE_ABORT, SDHC_CMD_CMDTYPE_MSK);
  } else if (DEF_BIT_IS_SET(p_cmd->Flags, SD_CARD_CMD_FLAG_DATA_RESUME)) {
    DEF_BIT_FIELD_WR(cmd, SDHC_CMD_CMDTYPE_RESUME, SDHC_CMD_CMDTYPE_MSK);
  } else if (DEF_BIT_IS_SET(p_cmd->Flags, SD_CARD_CMD_FLAG_DATA_SUSPEND)) {
    DEF_BIT_FIELD_WR(cmd, SDHC_CMD_CMDTYPE_SUSPEND, SDHC_CMD_CMDTYPE_MSK);
  }
  ...
  DEF_BIT_FIELD_WR(cmd, p_cmd->Cmd, SDHC_CMD_IX_MSK);
                                    ...
                                    CPU_BOOLEAN enable)
{
  ...
  if (enable == DEF_YES) {
    ...
    DEF_BIT_CLR(p_regs->INTSTATEN, SDHC_INTSIGEN_CARDINT);
    DEF_BIT_CLR(p_regs->INTSIGEN, SDHC_INTSIGEN_CARDINT);
  }
}
...
static void SD_CtrlrISR_Handler(SD_CARD_DRV *p_drv)
{
  ...
  if (DEF_BIT_IS_SET(intstatus, SDHC_INTSTAT_CARDINT)) {
    DEF_BIT_CLR(p_regs->INTSTATEN, SDHC_INTSTATEN_CARDINT);
  ...
  if (DEF_BIT_IS_SET(intstatus, SDHC_INTSTATEN_CARDINS)) {
    DEF_BIT_CLR(p_regs->INTSTATEN, SDHC_INTSTATEN_CARDINS);
  ...
  if (DEF_BIT_IS_SET(intstatus, SDHC_INTSTATEN_CARDREM)) {
    DEF_BIT_CLR(p_regs->INTSTATEN, SDHC_INTSTATEN_CARDREM);
