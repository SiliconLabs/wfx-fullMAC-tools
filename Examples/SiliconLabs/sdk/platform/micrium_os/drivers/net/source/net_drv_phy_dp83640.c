/***************************************************************************//**
 * @file
 * @brief Network Device Driver - Dp83640 Ethernet Phy
 *******************************************************************************
 * # License
...
#define  NET_PHY_INIT_RESET_RETRIES           10u               // Check for successful reset x times
...
#define  PHY_PAGESEL                        0x13u               // Page select reg.
#define  PHY_PTP_COC                        0x14u               // Page 6, PTP clock output control reg.
...
//                                                                 ------------ PHY_PAGESEL REGISTER BITS -------------
#define  PAGESEL_EXT_REG                     0x0u               // Extended reg.
#define  PAGESEL_PTP_CFG2                    0x6u               // PTP clock output control reg #2.

//                                                                 ------------ PHY_PTP_COC REGISTER BITS -------------
#define  PTP_CLKOUT_EN                DEF_BIT_15                // PTP clock output enabled.
...
const NET_PHY_API_ETHER NetPhy_API_DP83640 = {                  // PHY API fnct ptrs :
  .Init = &NetPhy_Init,
  .EnDis = &NetPhy_EnDis,
                        ...
                        RTOS_ERR *p_err)
{
  ...
  if (p_phy_cfg->BusMode == NET_PHY_BUS_MODE_RMII) {            // ------------- CONFIG RMII MASTER MODE --------------
                                                                // Sel page 6, PTP clock output control reg #2.
    p_dev_api->Phy_RegWr(p_if, phy_addr, PHY_PAGESEL, PAGESEL_PTP_CFG2, p_err);
    if (RTOS_ERR_CODE_GET(*p_err) != RTOS_ERR_NONE) {
      return;
    }
    //                                                             Rd ctrl reg, get PTP clock output en bit.
    p_dev_api->Phy_RegRd(p_if, phy_addr, PHY_PTP_COC, &reg_val, p_err);
    if (RTOS_ERR_CODE_GET(*p_err) != RTOS_ERR_NONE) {
      return;
    }

    reg_val &= ~PTP_CLKOUT_EN;                                  // Disable PTP clock output.
    p_dev_api->Phy_RegWr(p_if, phy_addr, PHY_PTP_COC, reg_val, p_err);
    if (RTOS_ERR_CODE_GET(*p_err) != RTOS_ERR_NONE) {
      return;
    }
    //                                                             Sel page 0, extended reg.
    p_dev_api->Phy_RegWr(p_if, phy_addr, PHY_PAGESEL, PAGESEL_EXT_REG, p_err);
    if (RTOS_ERR_CODE_GET(*p_err) != RTOS_ERR_NONE) {
      return;
    }
  }
                                ...
                                RTOS_ERR           *p_err)
{
  ...
  p_dev_api->IO_Ctrl(p_if,
                     ...
                     p_link_state,
                     &local_err);                               // !!!! Ignore dev update err.
}
