...
#ifndef  KERNEL_ISR_STACK_SIZE_DFLT
#define  KERNEL_ISR_STACK_SIZE_DFLT         256u
#endif

#ifndef  KERNEL_IDLE_TASK_STACK_SIZE_DFLT
#define  KERNEL_IDLE_TASK_STACK_SIZE_DFLT   256u
#endif

#ifndef  KERNEL_STAT_TASK_STACK_SIZE_DFLT
#define  KERNEL_STAT_TASK_STACK_SIZE_DFLT   256u
#endif

#ifndef  KERNEL_TICK_TASK_STACK_SIZE_DFLT
#define  KERNEL_TICK_TASK_STACK_SIZE_DFLT   256u
#endif

#ifndef  KERNEL_TMR_TASK_STACK_SIZE_DFLT
#define  KERNEL_TMR_TASK_STACK_SIZE_DFLT    256u
#endif
 ...
 *
 * Note(s) : (1) This option is deprecated. This option should not be used in a new design, since it will
 *               be removed in a future release. If a matching delay is necessary, it is recommended to
 *               manually perform the delay computation using the function OSTimeGet().
 *******************************************************************************************************/
...
#define  OS_OPT_TIME_MATCH                  ((OS_OPT)DEF_BIT_02)// DEPRECATED. See Note #1
 ...
 *
 * Note(s) : (1) The idle task will be removed in an upcoming release as well as the IdleTask
 *               configuration field. In case an Idle task is really needed, it is recommended to create
 *               a very low priority task (with no other tasks at the same priority) that never performs
 *               any blocking calls.
 *******************************************************************************************************/
...
struct os_init_cfg {                                            // Runtime Configuration
  ...
  OS_STACK_CFG IdleTask;                                        // DEPRECATED. See Note #1
...
extern CPU_STK * OS_CONST    OSCfg_IdleTaskStkBasePtr;
extern CPU_STK_SIZE OS_CONST OSCfg_IdleTaskStkLimit;
extern CPU_STK_SIZE OS_CONST OSCfg_IdleTaskStkSize;
extern CPU_INT32U OS_CONST   OSCfg_IdleTaskStkSizeRAM;
...
extern OS_PRIO OS_CONST      OSCfg_TickTaskPrio;
extern CPU_STK * OS_CONST    OSCfg_TickTaskStkBasePtr;
extern CPU_STK_SIZE OS_CONST OSCfg_TickTaskStkLimit;
extern CPU_STK_SIZE OS_CONST OSCfg_TickTaskStkSize;
extern CPU_INT32U OS_CONST   OSCfg_TickTaskStkSizeRAM;
 ...
 *                                            DEPRECATED ELEMENTS
 *
 *  Note(s) : (1) The elements in this section are deprecated. Unless already used, these elements should
 *                not be used, since they will be removed in a future release.
 *
 *            (2) The idle task will be removed in an upcoming release as well as the function
 *                OSIdleTaskHook() and OS_AppIdleTaskHookPtr. In case an Idle task is really needed, it is
 *                recommended to create a very low priority task (with no other tasks at the same
 *                priority) that never performs any blocking calls.
 *
 *            (3) The tick task will be removed in an upcoming release. It is not recommended to rely on
 *                the tick hook functions for new designs. The function OSTimeSet() will also be removed
 *                in an upcoming release.
 *
 *            (4) The OS_MEM module of the kernel will be removed in an upcoming release. Any new design
 *                should rely on the functionalities provided by the lib_mem library part of the common
 *                module.
 ********************************************************************************************************
 *******************************************************************************************************/
...
void OSIdleTaskHook(void);                                      // See note #2.
#if (OS_CFG_APP_HOOKS_EN == DEF_ENABLED)
OS_EXT OS_APP_HOOK_VOID OS_AppIdleTaskHookPtr;                  // See note #2.
#endif

void OSTimeTickHook(void);                                      // See note #3.
#if (OS_CFG_APP_HOOKS_EN == DEF_ENABLED)
OS_EXT OS_APP_HOOK_VOID OS_AppTimeTickHookPtr;                  // See note #3.
#endif

void OSTimeSet(OS_TICK  ticks,                                  // See note #3.
               RTOS_ERR *p_err);

/****************************************************************************************************//**
 *                                   FIXED SIZE MEMORY BLOCK MANAGEMENT
 * @addtogroup KERNEL_MEMMNG
 * @{
 *******************************************************************************************************/

#if (OS_CFG_MEM_EN == DEF_ENABLED)
void OSMemCreate(OS_MEM      *p_mem,                            // See note #4.
                 CPU_CHAR    *p_name,
                 void        *p_addr,
                 OS_MEM_QTY  n_blks,
...
void *OSMemGet(OS_MEM   *p_mem,                                 // See note #4.
               RTOS_ERR *p_err);

void OSMemPut(OS_MEM   *p_mem,                                  // See note #4.
              void     *p_blk,
              RTOS_ERR *p_err);
#endif
...
#else
#if   ((OS_CFG_SCHED_ROUND_ROBIN_EN == DEF_ENABLED) \
  && (OS_CFG_TASK_TICK_EN == DEF_DISABLED))
#error  "OS_CFG.H, OS_CFG_TASK_TICK_EN must be enabled to use the Round Robin Scheduler."
#endif
#endif
...
#ifndef OS_CFG_DYN_TICK_EN
#error  "OS_CFG.H, Missing OS_CFG_DYN_TICK_EN: Determines whether to use the dynamic ticking feature"
#else
#if   ((OS_CFG_DYN_TICK_EN == DEF_ENABLED) \
  && (OS_CFG_TASK_TICK_EN == DEF_DISABLED))
#error  "OS_CFG.H, OS_CFG_TASK_TICK_EN must be enabled to use the dynamic ticking feature."
#endif
#endif
...
#if   ((OS_CFG_STAT_TASK_EN == DEF_ENABLED) \
  && (OS_CFG_TASK_TICK_EN == DEF_DISABLED))
#error  "OS_CFG.H, OS_CFG_TASK_TICK_EN must be enabled to use the Statistics Task."
#endif
#endif
...
#if   ((OS_CFG_TMR_EN == DEF_ENABLED) \
  && (OS_CFG_TASK_TICK_EN == DEF_DISABLED))
#error  "OS_CFG.H, OS_CFG_TASK_TICK_EN must be enabled to use Timers."
#endif
#endif
