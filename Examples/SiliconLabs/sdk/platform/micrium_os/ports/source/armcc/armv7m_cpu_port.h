 ...
 *                           }
 *
 * @note     (4) (a) The CPU CRITICAL sections are deprecated and now rely on the em_core CRITICAL sections
 *                   from the em_core.h file. The user should call directly em_core CRITICAL sections.
 *                   These sections prevent your code to be interrupted by any ISR or exception by setting
 *                   or clearing the PRIMASK register.
 *
 *                   Example :
 *                   void  Fnct (void)
 *                   {
 ...
 *                   }
 *
 *               (b) If the user doesn't wants to disable every ISRs and exceptions, em_core ATOMIC section
 *                   macros should be used. These sections prevent your code to be interrupted by any ISR
 *                   with a priority below the BASEPRI register value. But The user will have to take care
 *                   of setting the ISR priorities and the BASEPRI value accordingly.
 *******************************************************************************************************/
//                                                                 Configure CPU critical method      (see Note #1) :
#define  CPU_CFG_CRITICAL_METHOD    CPU_CRITICAL_METHOD_STATUS_LOCAL

typedef CPU_INT32U CPU_SR;                                      // Defines   CPU status register size.

//                                                                 Allocates CPU status register word (see Note #3a).
#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_LOCAL)
#define  CPU_SR_ALLOC()             CORE_DECLARE_IRQ_STATE
#else
#define  CPU_SR_ALLOC()
#endif

#define  CPU_INT_DIS()          CORE_ENTER_CRITICAL()           // Save    CPU status word & disable interrupts.
#define  CPU_INT_EN()           CORE_EXIT_CRITICAL()            // Restore CPU status word.

#ifdef   CPU_CFG_INT_DIS_MEAS_EN
//                                                                 Disable interrupts, ...
//                                                                 & start interrupts disabled time measurement.
#define  CPU_CRITICAL_ENTER()  do { CORE_ENTER_CRITICAL(); \
                                    CPU_IntDisMeasStart(); }  while (0)
//                                                                 Stop & measure   interrupts disabled time,
//                                                                 ...  & re-enable interrupts.
#define  CPU_CRITICAL_EXIT()   do { CPU_IntDisMeasStop(); \
                                    CORE_EXIT_CRITICAL(); }  while (0)

#else

#define  CPU_CRITICAL_ENTER()   CORE_ENTER_CRITICAL()           // Disable   interrupts.
#define  CPU_CRITICAL_EXIT()    CORE_EXIT_CRITICAL()            // Re-enable interrupts.
...
#define  CPU_BREAK()    __breakpoint(55)
...
void CPU_IntDis(void);                                          // em_core CRITICAL sections should be used
void CPU_IntEn(void);                                           // to enable and disable interrupts.

CPU_SR CPU_SR_Save(void);
void CPU_SR_Restore(CPU_SR cpu_sr);

void CPU_WaitForInt(void);                                      // cmsis intrinsic function __WFI() should be used.
void CPU_WaitForExcept(void);                                   // cmsis intrinsic function __WFE() should be used.
