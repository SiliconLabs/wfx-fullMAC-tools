...
#include "cmsis_os.h"
#else
#error "Console: OS definition missing"
#endif
#endif

#include "console.h"

int console_sem_create(void);
int console_sem_wait(uint32_t timeout_sec);
int console_sem_release(void);
...
int console_sem_wait (uint32_t timeout_sec)
{
  ...
  OSSemSet(&console_sem, 0, &err);
  OSSemPend(&console_sem,
            OSCfg_TickRate_Hz * timeout_sec,
...
#else
#ifdef FREERTOS
static osSemaphoreId console_sem;
...
int console_sem_create (void)
{
  ...
  osSemaphoreDef(sem);
  console_sem = osSemaphoreCreate(osSemaphore(sem), 1);
  if (console_sem != NULL) {
    ret = osSemaphoreWait(console_sem, 0);
  }
  return ret;
}
...
int console_sem_wait (uint32_t timeout_sec)
{
  ...
  if (timeout_sec > 0) {
    nb_ticks = configTICK_RATE_HZ * timeout_sec;
  } else {
    // Wait forever
    nb_ticks = portMAX_DELAY;
  }

  return osSemaphoreWait(console_sem, nb_ticks);
}
...
int console_sem_release (void)
{
  ...
  if (osSemaphoreRelease(console_sem) == osOK) {
    ret = 0;
  }
  return ret;
}
