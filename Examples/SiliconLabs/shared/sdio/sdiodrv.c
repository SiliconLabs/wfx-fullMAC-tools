...
static void config_gpio (SDIODRV_Init_t *init)
{
  ...
  GPIO_PinModeSet((GPIO_Port_TypeDef)AF_SDIO_CLK_PORT(init->portLocationClk),
                  AF_SDIO_CLK_PIN(init->portLocationClk),
                  gpio_mode,
                  1);

  gpio_mode = init->portLocationCmd == 0 ?
              gpioModePushPull : gpioModePushPullAlternate;
  GPIO_PinModeSet((GPIO_Port_TypeDef)AF_SDIO_CMD_PORT(init->portLocationCmd),
                  AF_SDIO_CMD_PIN(init->portLocationCmd),
                  gpio_mode,
                  1);

  gpio_mode = init->portLocationDat == 0 ?
              gpioModePushPull : gpioModePushPullAlternate;
  GPIO_PinModeSet((GPIO_Port_TypeDef)AF_SDIO_DAT0_PORT(init->portLocationDat),
                  AF_SDIO_DAT0_PIN(init->portLocationDat),
                  gpio_mode,
                  1);
  ...
  if ((init->transferWidth == SDIO_TRANSFER_WIDTH_4BIT)
    ...
    GPIO_PinModeSet((GPIO_Port_TypeDef)AF_SDIO_DAT1_PORT(init->portLocationDat),
                    AF_SDIO_DAT1_PIN(init->portLocationDat),
                    gpio_mode,
                    1);

    GPIO_PinModeSet((GPIO_Port_TypeDef)AF_SDIO_DAT2_PORT(init->portLocationDat),
                    AF_SDIO_DAT2_PIN(init->portLocationDat),
                    gpio_mode,
                    1);

    GPIO_PinModeSet((GPIO_Port_TypeDef)AF_SDIO_DAT3_PORT(init->portLocationDat),
                    AF_SDIO_DAT3_PIN(init->portLocationDat),
                    gpio_mode,
                    1);
...
static uint32_t send_cmd0_go_idle_state (SDIODRV_Handle_t *handle)
{
  SDIO_Cmd_t cmd0 = {
    ...
    .respType = SDIO_RESP_TYPE_RESP48, // Some cards answer to this command
    .checkCrc = true,
    .checkIndex = true,
    .enableDma = false,
    .multiblocks = false
