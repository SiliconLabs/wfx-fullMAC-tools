 ...
 * 2^48.
 */
#define RESEED_INTERVAL ((uint64_t)1 << 48)
#define DRBG_HASH_MAX_DIGEST_SIZE SHA512_DIGESTSIZE
 ...
 */
static uint32_t _sx_drbg_hash_df(sx_drbg_hash_ctx_t *ctx,
      ...
      block_t output)
{
   ...
   for (size_t i = 0; i < len; i++) {
      ...
      block_t hash_input[SX_HASH_ARRAY_MAX_ENTRIES] = {
         block_t_convert(&counter, sizeof(counter)),
         block_t_convert(&no_of_bits, sizeof(no_of_bits))
      };
      ...
      uint8_t *nonce, size_t nonce_size)
{
   ...
   block_t array_enp[] = {
       BLOCK_T_CONV(entropy, entropy_size),
       BLOCK_T_CONV(nonce, nonce_size),
       BLOCK_T_CONV(personalization, personalization_size)
   };
 ...
 * @return CRYPTOLIB_SUCCESS when execution was successful
 */
static void _sx_drbg_hash_hashgen(sx_drbg_hash_ctx_t *ctx, block_t out)
{
   CRYPTOLIB_ASSERT_NM(ctx);
   ...
   for (int i = 0; i < m; i++) {
      ...
      sx_math_array_incr(data, ctx->seedlen, 1);
   }
   if (out.len % ctx->hash_block_size) {
      uint8_t tempo_data[DRBG_HASH_MAX_DIGEST_SIZE] = { 0 };
      block_t tempo = BLOCK_T_CONV(tempo_data, ctx->hash_block_size);
      uint32_t ret = sx_hash_blk(ctx->hash_fct, data_block, tempo);
      CRYPTOLIB_ASSERT_NM(ret == CRYPTOLIB_SUCCESS);
      /* hashgen 5: returned_bits = leftmost (requested_no_of_bits) bits of W */
      memcpy_blk(out, tempo, out.len);
   }
}
...
uint32_t sx_drbg_hash_generate(sx_drbg_hash_ctx_t *ctx, block_t out)
{
   ...
   if (ctx->reseed_counter > RESEED_INTERVAL) {
      ...
      return CRYPTOLIB_DRBG_RESEED_NEEDED;
   }

   if (out.len > MAX_REQUEST_SIZE) {
      CRYPTOLIB_PRINTF("DRBG_Hash: Requested %d bytes, but max size is %d\n",
            out.len, MAX_REQUEST_SIZE);
      return CRYPTOLIB_INVALID_PARAM;
   }

   /* 3. returned_bits = Hashgen(requested_number_of_bits, V) */
   _sx_drbg_hash_hashgen(ctx, out);
   ...
   sx_math_array_incr(ctx->V, ctx->seedlen, ctx->reseed_counter);
