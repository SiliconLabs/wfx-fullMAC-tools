                ...
                size_t add_len )
{
    ...
    ctx->dir = mode == MBEDTLS_AES_ENCRYPT ? ENC : DEC_WITH_TAG;
    ctx->add_len    = add_len;

    ctx->sx_ctx_blk.addr  = ctx->sx_ctx;
    ctx->sx_ctx_blk.len   = sizeof(ctx->sx_ctx);
    ctx->sx_ctx_blk.flags = 0;
    ...
    if (add_len == 0) {
        ...
        sx_ret = sx_aes_blk(GCM, ctx->dir, CTX_BEGIN, key, NULL_blk, nonce,
                            NULL_blk, NULL_blk, aad, NULL_blk, ctx->sx_ctx_blk,
                            NULL_blk);
                ...
                unsigned char *output )
{
    ...
    if (ctx->add_len == 0 && ctx->len == 0) {
        ...
        nonce.addr      = (uint8_t*)ctx->iv;
        nonce.len       = AES_IV_GCM_SIZE;
        nonce.flags     = 0;
        ...
        /* Execute GCM operation */
        sx_ret = sx_aes_blk(GCM, ctx->dir, CTX_BEGIN, key, NULL_blk, nonce,
                            data_in,  data_out, NULL_blk, NULL_blk,
                            ctx->sx_ctx_blk, NULL_blk);
        cryptoacc_management_release();
    }
    else {
        status = cryptoacc_management_acquire();
        ...
        /* Execute GCM operation */
        sx_ret = sx_aes_blk(GCM, ctx->dir, CTX_MIDDLE, key, NULL_blk, ctx->sx_ctx_blk,
                            data_in,  data_out, NULL_blk, NULL_blk,
                            ctx->sx_ctx_blk, NULL_blk);
        cryptoacc_management_release();
    }
                ...
                size_t tag_len )
{
    ...
    if (ctx->add_len == 0 && ctx->len == 0) {
        ...
        return mbedtls_gcm_crypt_and_tag( ctx,
                                          ...
                                          MBEDTLS_GCM_DECRYPT,
                                          0, ctx->iv, AES_IV_GCM_SIZE, 0, 0, 0, 0,
                                          tag_len, tag);
    }
    else {
        key = block_t_convert(ctx->key, ctx->keybits / 8);
        ...
        /* Execute GCM operation */
        sx_ret = sx_aes_blk(GCM, ctx->dir, CTX_END, key, NULL_blk, ctx->sx_ctx_blk,
                            NULL_blk, NULL_blk, NULL_blk, _tag, NULL_blk,
                            lena_lenc_blk);
        cryptoacc_management_release();
        ...
        if ( ctx->dir == DEC_WITH_TAG ) {
            // For decryption, verify tag, in "constant-time".
                       ...
                       unsigned char *tag )
{
    int status;
    uint32_t sx_ret;
    sx_aes_mode_t dir = mode == MBEDTLS_AES_ENCRYPT ? ENC : DEC_WITH_TAG;
    block_t key;
    block_t aad;
    block_t _tag;
    block_t nonce;
    ...
    /* Execute GCM operation */
    sx_ret = sx_aes_blk(GCM, dir, CTX_WHOLE, key, NULL_blk, nonce,
                        data_in, data_out, aad, _tag, NULL_blk, NULL_blk);
    cryptoacc_management_release();
                      ...
                      unsigned char *output )
{
    ...
    /* Execute GCM operation */
    sx_ret = sx_aes_blk(GCM, DEC_WITH_TAG, CTX_WHOLE, key, NULL_blk, nonce,
                        data_in, data_out, aad, _tag, NULL_blk, NULL_blk);
    cryptoacc_management_release();
