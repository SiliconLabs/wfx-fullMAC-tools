...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  if (result == SL_STATUS_OK) {
    ...
    bus_tx_frame.data_length = framelength + padding;
    bus_tx_frame.frame = tx_buffer;
    bus_tx_frame.interface = interface;
    bus_tx_frame.priority = 0;

    OSFlagPost(&bus_events, SL_WFX_BUS_EVENT_FLAG_TX, OS_OPT_POST_FLAG_SET, &err);
    OSSemPend(&bus_tx_complete_sem, 0, OS_OPT_PEND_BLOCKING, 0, &err);
    result = SL_STATUS_OK;
