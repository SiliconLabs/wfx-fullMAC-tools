...
#define RF_TEST_AGENT_CMD_NAME      "wfx_test_agent"
                                  ...
                                  char **argv)
{
  ...
  if ((argc >= 2)
      && (strncmp(RF_TEST_AGENT_CMD_NAME, argv[0], strlen(RF_TEST_AGENT_CMD_NAME)) == 0)
      && (argv[0][strlen(RF_TEST_AGENT_CMD_NAME)] == '\0')) {

    if ((strncmp(TX_TEST_KEY_WORD, argv[1], strlen(TX_TEST_KEY_WORD)) == 0)
        && (argv[1][strlen(TX_TEST_KEY_WORD)] == '\0')
        && (argc == 3)) {

      /* Start TX RF test */
      status = sl_wfx_send_configuration(argv[2], strlen(argv[2]));
    ...
    } else if ((strncmp(RX_TEST_KEY_WORD, argv[1], strlen(RX_TEST_KEY_WORD)) == 0)
               && (argv[1][strlen(RX_TEST_KEY_WORD)] == '\0')) {
    ...
    } else if ((strncmp(FW_VERSION_KEY_WORD, argv[1], strlen(FW_VERSION_KEY_WORD)) == 0)
               && (argv[1][strlen(FW_VERSION_KEY_WORD)] == '\0')) {
    ...
    } else if ((strncmp(DRIVER_VERSION_KEY_WORD, argv[1], strlen(DRIVER_VERSION_KEY_WORD)) == 0)
               && (argv[1][strlen(DRIVER_VERSION_KEY_WORD)] == '\0')) {

      /* Return driver version */
      printf("%s\r\n", FMAC_DRIVER_VERSION_STRING);
      status = SL_STATUS_OK;

    }else if ((strncmp(TEST_AGENT_VERSION_KEY_WORD, argv[1], strlen(TEST_AGENT_VERSION_KEY_WORD)) == 0)
              && (argv[1][strlen(TEST_AGENT_VERSION_KEY_WORD)] == '\0')) {
