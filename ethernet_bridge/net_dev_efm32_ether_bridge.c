/***************************************************************************//**
 * @file net_dev_efm32_ether_bridge.c
 * @brief Network Device Driver - A customized EFM32GG11 Ethernet Driver 
 *        for Bridge Function based on GEM driver (net_drv_ether_gem.c)
 *******************************************************************************
 * # License
...
#define  GEM_BIT_CFG_RX1536BYTEFRAMES         DEF_BIT_08        // Unicast hash enable.
...
const NET_DEV_API_ETHER NetDev_API_EFM32_ETH = {                      // Ether DMA dev API fnct ptrs :
...
//                             | ((CPU_INT32U)hw_addr[5] << (1 * DEF_INT_08_NBR_BITS)));
  ...
  p_dev->NET_CFG |= GEM_BIT_CFG_FULL_DUPLEX                     // Full speed by default.
                    //| GEM_BIT_CFG_GIGE_EN
                    | GEM_BIT_CFG_RX1536BYTEFRAMES | GEM_BIT_CFG_COPY_ALL;
                      ...
                      RTOS_ERR   *p_err)
{
  ...
  *p_data = (CPU_INT08U *)(addr & GEM_RXBUF_ADDR_MASK);         // Return a pointer to the newly received data area.
  ...
  NetDev_RxDescPtrCurInc(p_if);                                 // Free the current descriptor.
  LOG_TRACE("Forward frame to WF200\r\n");
  
  low_level_output_ethernet(*p_data, *p_size);
  NetBuf_FreeBufDataAreaRx(p_if->Nbr, *p_data);
  *p_size = 0u;
  *p_data = DEF_NULL;
  RTOS_ERR_SET(*p_err, RTOS_ERR_RX);
  return;
