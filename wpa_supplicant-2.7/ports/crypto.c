...
/**
 * @brief               the function to initialize the mbedtls CTR-DRBG module to enable the random number generator.
 * 
 * @param p_entropy_ctx the pointer to the context of the entropy source.
 * @param p_ctr_drbg    the pointer to the context of the CTR-DRBG module.
 * @return 0            the mbedtls CTR-DRBG module initialization is successful
 * @return non-zero     the mbedtls CTR-DRBG module initialization is failed 
 */
static int crypto_ctr_drbg_init(mbedtls_entropy_context* p_entropy_ctx, mbedtls_ctr_drbg_context* p_ctr_drbg_ctx)
{
    int ret;
    ...
    if (!random_init) 
    {
        ...
        mbedtls_entropy_init(p_entropy_ctx);
        /* initialize the CTR-DRBG module */
        mbedtls_ctr_drbg_init(p_ctr_drbg_ctx);
        /* seed and setup entropy source for future reseeds */
        ret = mbedtls_ctr_drbg_seed(p_ctr_drbg_ctx, mbedtls_entropy_func, p_entropy_ctx, NULL, 0);
        if(ret)
        {
            mbedtls_entropy_free(p_entropy_ctx);
            ret = -1;
        }
        random_init = true;
    }
...
/**
 * PRNG compatible with Mbed TLS
 */
static int mbedtls_get_random(
    void *ctx,
    unsigned char *buf,
    size_t len)
{
...
struct crypto_bignum * crypto_bignum_init_uint(
    unsigned int val)
{
    mbedtls_mpi* mpi;
    int ret;
    ...
    struct crypto_bignum* d)
{
    ...
    if (mpi_a && mpi_b && mpi_c && mpi_d) {
        ...
        ret = mbedtls_mpi_exp_mod(&mpi_res, mpi_a, mpi_b, mpi_c, NULL);
        if (!ret)
        {
            mbedtls_mpi_copy(mpi_d, &mpi_res);
        }
    ...
    const struct crypto_bignum *m)
{
    ...
    len = mbedtls_mpi_size((const mbedtls_mpi*)m);
    ret = mbedtls_mpi_fill_random((mbedtls_mpi*)r, len, mbedtls_get_random, NULL);
    if (!ret)
    {
        ret = mbedtls_mpi_mod_mpi((mbedtls_mpi*)r, (const mbedtls_mpi*)r, (const mbedtls_mpi*)m);
    }
    ...
    const struct crypto_bignum *a)
{
    ...
    return (mbedtls_mpi_lsb((const mbedtls_mpi*)a) == 0);
}
...
int crypto_bignum_addmod(
    ...
    struct crypto_bignum *d)
{
    const mbedtls_mpi* mpi_a = (const mbedtls_mpi*)a;
    const mbedtls_mpi* mpi_b = (const mbedtls_mpi*)b;
    const mbedtls_mpi* mpi_c = (const mbedtls_mpi*)c;
    mbedtls_mpi* mpi_d = (mbedtls_mpi*)d;
    int ret = -1;

    if (mpi_a && mpi_b && mpi_c && mpi_d)
    {
        ret = mbedtls_mpi_add_mpi(mpi_d, mpi_a, mpi_b);
        if (!ret) {
            ret = mbedtls_mpi_mod_mpi(mpi_d, mpi_d, mpi_c);
        }
    }
    ...
    struct crypto_bignum *c)
{
    ...
    if (mpi_a && mpi_b && mpi_c)
    {
        ret = mbedtls_mpi_mul_mpi(mpi_c, mpi_a, mpi_a);
        if (!ret) {
            ret = mbedtls_mpi_mod_mpi(mpi_c, mpi_c, mpi_b);
        }
    }
...
size_t crypto_ec_order_len(
    struct crypto_ec *e)
{
    mbedtls_ecp_group* ec = (mbedtls_ecp_group*)e;

    if (ec) {
        return mbedtls_mpi_size(&ec->N);
    }
...
const struct crypto_bignum * crypto_ec_get_a(struct crypto_ec *e)
{
    ...
    if (ec)
    {
        return (const struct crypto_bignum*)&ec->A;
    }
...
const struct crypto_bignum * crypto_ec_get_b(struct crypto_ec *e)
{
    ...
    if (ec)
    {
        return (const struct crypto_bignum*)&ec->B;
    }
    ...
    struct crypto_ec_point* res)
{
    ...
    crypto_ctr_drbg_init(&entropy_ctx, &ctr_drbg_ctx);
    if (ec && ecp_p && mpi_b && ecp_res) {
        ret = mbedtls_ecp_mul(ec, ecp_res, mpi_b, ecp_p, mbedtls_ctr_drbg_random, &ctr_drbg_ctx);
    }
